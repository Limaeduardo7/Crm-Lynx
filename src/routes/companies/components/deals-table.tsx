...
export const CompanyDealsTable: FC<Props> = ({ style }) => {
    ...
    return (
        <Card
            ...
            extra={
                <>
                    <Text className="tertiary">{t('deals.totalDealAmount')}</Text>
                    {isLoadingCompany ? (
        ...
        >
            {!hasData && (
                ...
                >
                    ...
                    <Link to={listUrl("deals")}>
                        ...
                        />{" "}
                        {t('deals.addDealsThroughSalesPipeline')}
                    </Link>
                </Space>
            )}
            ...
            {hasData && (
                ...
                >
                    <Table.Column
                        title={t('deals.dealTitleTitle')}
                        dataIndex="title"
                        filterIcon={<SearchOutlined />}
                        filterDropdown={(props) => (
                            <FilterDropdown {...props}>
                                <Input placeholder={t('deals.searchTitlePlaceholder')} />
                            </FilterDropdown>
                        )}
                    />
                    <Table.Column<Deal>
                        title={t('deals.dealAmountTitle')}
                        dataIndex="value"
                        sorter
                        render={(_, record) => {
                            return (
                        ...
                        filterDropdown={(props) => (
                            <FilterDropdown {...props}>
                                <Select
                                    ...
                                    style={{ width: "200px" }}
                                    mode="multiple"
                                    placeholder={t('deals.selectStagePlaceholder')}
                                />
                            </FilterDropdown>
                        )}
                    />
                    <Table.Column<Deal>
                        ...
                        title={t('deals.participantsTitle')}
                        render={(_, record) => {
                            return (
                                <Participants
                        ...
                        filterDropdown={(props) => {
                            return (
                                <FilterDropdown {...props}>
                                    <Select
                                        style={{ width: "200px" }}
                                        placeholder={t('deals.selectSalesOwnerPlaceholder')}
                                        {...usersSelectProps}
                                    />
                                </FilterDropdown>
                            );
