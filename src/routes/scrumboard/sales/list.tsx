...
export const SalesPage: FC<PropsWithChildren> = ({ children }) => {
    ...
    const stageGrouped = useMemo(() => {
        ...
        const stageAll = grouped.filter(
            (stage) => {
                return ![t('WON'), t('LOST')].includes(stage.title);
            }        );
    ...
    const handleDeleteStage = (args: { stageId: string }) => {
        deleteStage({
            ...
            successNotification: () => ({
                key: "delete-stage",
                type: "success",
                message: t("deleteStage.successMessage"),
                description: t("deleteStage.successDescription")
            }),
        });
    };
